<?php

return [
    'web' => [
        'lavanda' => 'Лаванда',
        'Любовь_Выбор_Страх_Надежда' => 'Любовь. Выбор. Страх. Надежда.',
        'Проходят_столетия_но_то_что_действительно_важно_человеку_остаётся_неизменным' => 'Проходят столетия, но то, что действительно важно человеку, остаётся неизменным',
        'Что_я_чувствую|Что_я_на_самом_деле_хочу|Куда_двигаться_дальше|Какое_решение_будет_правильным|Какими_могут_быть_наши_отношения|Когда_придет_время' => 'Что я чувствую|Что я на самом деле хочу|Куда двигаться дальше|Какое решение будет правильным|Какими могут быть наши отношения|Когда придет время',

    ],
    'auth' => [
        'login' => 'Вход',

    ],
    'admin' => [
        'dashboard' =>  'Дашборд',
        'this_is_the_dashboard_page' => 'Добро пожаловать в дашборд',
        'appearance' => 'Тема',
        'profile' =>  'Профиль',
        'password' =>  'Пароль',
        'language' =>  'Язык',
        'language_settings' =>  'Выбор языка',
        'change_your_account_locale' =>  'Сменить язык профиля',
        'platform' =>  'Платформа',
        'admin_panel' =>  'Админ панель',
        'manage_your_profile_and_account_settings' => 'Сменить настройки личного профиля пользователя',
        'russian' => 'Русский',
        'english' => 'Английский',
        'serbian' => 'Сербский',
        'german' => 'Немецкий',
        'update_your_account_theme' => "Сменить тему профиля",
        'appearance_settings' => 'Настройки темы',
        'system' => 'системная',
        'light' => 'светлая',
        'dark' => 'темная',
        'password_settings' => 'Настройки пароля',
        'update_password' => 'Изменить пароль',
        "password_updated_successfully" => "Пароль успешно изменен!",
        "please_enter_the_correct_current_password" => "Пожалуйста введите правильный пароль",
        'ensure_your_account_is_using_a_long_random_password_to_stay_secure' => 'Используйте надежный пароль дабы предотвратить взлом вашего аккаунта',
        'current_password' => 'Текущий пароль',
        'confirm_password' => 'Повторите пароль',
        'new_password' => 'Новый пароль',
        'save_password' => 'Сохранить',
        'update_your_name_and_email_address' => 'Изменить имя пользователя или электронную почту',
        'email_address' => 'Электронная почта',
        'save' => 'Сохранить',
        'full_name' => 'Имя пользователя',
        'settings' => 'Настройки',
        'log_out' => 'Выйти',
        'log_in_to_your_account' => 'Вход в личный кабинет',
        'enter_your_email_and_password_below_to_log_in' => 'Введите емаил и пароль',
        'enter_your_password' => 'Введите пароль',
        'log_in' => 'Войти',
        'cards' => 'Карты',
        'user_successfully_updated' => 'Информация профиля успешно обновлена!',
        'tarot' => 'Таро',
        'lenormand' => 'Ленорманд',
        'mind_games' => 'Игры разума',
        'bonus' => 'Бонус',
        'metaphoric' => 'Метафорические',
        'select_a_card_category' => 'Выберите категорию карт',
        'select_category' => 'Выберите категорию',
        'delete_element' => 'Удалить элемент',
        'delete' => 'Удалить',
        'edit' => 'Редактировать',
        'card' => 'карта',
        'are_you_sure_you_want_to_delete_this_element_this_action_cannot_be_undone' => 'Вы точно уверены что хотите удалить данный элемент? Его нельзя будет восстановить.',
        'create_new' => 'Добавить новую',
        'here_you_can_manage_the_cards' => 'Здесь вы можете редактировать карты',
        "these_credentials_dont_match_our_records" => "Неправильные имя пользователя или пароль",
        "welcome" => "Добро пожаловать",
        "select_language" => "Выберите язык",
        "please_wait_for_1-2_minutes_in_order_to_see_updated_image_files" => "Фото буду обработаны в течение 1-2 минут",
        "image_successfully_updated" => "Фото успешно обновлено!",
        "image_successfully_created" => "Фото успешно создано!",
        "card_successfully_created" => "Карта успешно создана!",
        "card_successfully_updated" => "Карта успешно обновлена!",
        "card_successfully_deleted" => "Карта успешно удалена!",
        "rune_successfully_created" => "Руна успешно создана!",
        "rune_successfully_updated" => "Руна успешно обновлена!",
        "rune_successfully_deleted" => "Руна успешно удалена!",
        "faq_successfully_created" => "FAQ успешно создано!",
        "faq_successfully_updated" => "FAQ успешно обновлено!",
        "faq_successfully_deleted" => "FAQ успешно удалено!",
        "item_successfully_created" => "Элемент успешно создан!",
        "item_successfully_updated" => "Элемент успешно обновлен!",
        "item_successfully_deleted" => "Элемент успешно удален!",
        "iching_successfully_updated" => "Ицзин успешно обновлен!",
        "audio_successfully_created" => "Аудио успешно создано!",
        "audio_successfully_updated" => "Аудио успешно обновлено!",
        "audio_successfully_deleted" => "Аудио успешно удалено!",
        "theme_successfully_created" => "Тема успешно создана!",
        "theme_successfully_updated" => "Тема успешно обновлена!",
        "theme_successfully_deleted" => "Тема успешно удалена!",
        "affirmation_successfully_created" => "Аффирмация успешно создана!",
        "affirmation_successfully_updated" => "Аффирмация успешно обновлена!",
        "affirmation_successfully_deleted" => "Аффирмация успешно удалена!",
        "article_successfully_created" => "Статья успешно создана!",
        "article_successfully_updated" => "Статья успешно обновлена!",
        "article_successfully_deleted" => "Статья успешно удалена!",
        "set_successfully_created" => "Комбинация успешно создана!",
        "set_successfully_updated" => "Комбинация успешно обновлена!",
        "set_successfully_deleted" => "Комбинация успешно удалена!",
        "create_a_card" => "Создать карту",
        "card_name" => "Название карты",
        "card_description" => "Описание карты",
        "theme_name" => "Название темы",
        "create_a_rune" => "Создать руну",
        "rune_name" => "Название руны",
        "add_new" => "Добавить",
        "there_are_no_cards_here_yet" => "Здесь пока нет ни одной карты...",
        "there_are_no_runes_here_yet" => "Здесь пока нет ни одной руны...",
        "there_are_no_ichings_here_yet" => "Здесь пока нет ни одного Ицзин...",
        "there_are_no_audios_here_yet" => "Здесь пока нет ни одного аудио...",
        "there_are_no_items_here_yet" => "Здесь пока нет ни одного элемента...",
        "there_are_no_affirmations_here_yet" => "Здесь пока нет ни одной аффирмации...",
        "there_are_no_articles_here_yet" => "Здесь пока нет ни одной статьи...",
        "there_are_no_legals_here_yet" => "Здесь пока нет ни одного документа...",
        "front_image" => "Передняя сторона",
        "back_image" => "Задняя сторона",
        "image" => "Фото",
        "card_advice" => "Совет карты",
        "card_meaning" => "Интерпретация карты",
        "meaning" => "Интерпретация",
        "rune_advice" => "Совет руны",
        "rune_meaning" => "Интерпретация руны",
        "image_alternative_text" => "Текстовое описание фото",
        "a_squirrel_is_sitting_on_a_tree" => "Белка сидит на дереве...",
        "update_image_alt" => "Изменить описание фото",
        "edit_a_card" => "Редактировать карту",
        "edit_a_rune" => "Редактировать руну",
        "edit_affirmation" => "Редактировать аффирмацию",
        "create_affirmation" => "Создать аффирмацию",
        "edit_article" => "Редактировать статью",
        "edit_legal" => "Редактировать документ",
        "create_article" => "Создать статью",
        "cancel" => "Отмена",
        "backside" => "Рубашка карты",
        "current_files" => "Текущие файлы",
        "question" => "Вопрос",
        "answer" => "Ответ",
        "faqs" => "FAQs",
        "edit_faq" => "Редактировать FAQ",
        "create_faq" => "Создать FAQ",
        "runes" => "Руны",
        "rune_images" => "Фотографии руны",
        "love_and_relationship" => "Любовь и отношения",
        "general" => "Общая",
        "career" => "Карьера",
        "rune" => "Руна",
        "categories" => "Категории",
        "category" => "категорию",
        "theme" => "тему",
        "themes" => "Темы",
        "subsections" => "Подразделы",
        "iching" => "Ицзин",
        "edit_iching" => "Редактировать Ицзин",
        "edit_item" => "Редактировать элемент",
        "create_item" => "Добавить элемент",
        "delete_item" => "Удалить элемент",
        "description" => "Описание",
        "practice" => "Практика",
        "item_description" => "Описание элемента",
        "audios" => "Аудио",
        "edit_audio" => "Редактировать аудио",
        "item_name" => "Название элемента",
        "add" => "Добавить",
        "file" => "Файл",
        "audio_description" => "Описание аудио",
        "audio_file" => "Аудио файл",
        "create_theme" => "Создать тему",
        "update_theme" => "Обновить тему",
        "edit_rune" => "Редактировать руну",
        "edit_card" => "Редактировать карту",
        "affirmations" => "Аффирмации",
        "affirmation_category" => "Категория",
        "quote" => "Цитата",
        "no_categories" => "Категорий нет",
        "preview" => "Превью",
        "articles" => "Статьи",
        "article" => "Статья",
        "useful_resources" => "Полезные ресурсы",
        "stone" => "Камень",
        "stones" => "Камни",
        "stone_successfully_created" => "Камень успешно создан!",
        "stone_successfully_updated" => "Камень успешно обновлен!",
        "stone_successfully_deleted" => "Камень успешно удален!",
        "create_stone" => "Создать камень",
        "delete_stone" => "Удалить камень",
        "stone_meaning" => "Значение камня",
        "stone_name" => "Название камня",
        "there_are_no_stones_here_yet" => "Пока не добавлено ни одного камня...",
        "edit_stone" => "Редактировать камень",
        "stone_images" => "Фото камня",
        "stone_preview" => "Второстепенное фото камня",
        "stone_image" => "Главное фото камня",
        "article_name" => "Название статьи",
        "create_match_set" => "Создать комбинацию",
        "delete_match_set" => "Удалить комбинацию",
        "edit_match_set" => "Редактировать комбинацию",
        "match_set_meaning" => "Толкование комбинации",
        "match_set_advice" => "Совет комбинации",
        "select_match_set_category" => "Выберите категорию комбинаций",
        "match_sets" => "Комбинации",
        "match_set" => "Комбинация",
        "there_are_no_match_sets_here_yet" => "Пока не добавлено ни одной комбинации...",
        "select_items_for_the_set" => "Выберите элементы для комбинации",
        "user_agreement" => "Согласие на обработку персональных данных",
        "privacy_policy" => "Пользовательское соглашение",
        "legal_successfully_updated" => "Документ успешно обновлен!",
        "legals" => "Соглашения",
        "legal" => "Соглашение",
        "content" => "Содержание",
    ]
];
