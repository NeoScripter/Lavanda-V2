...
<li class="grid max-w-140 gap-6 text-sm">
    <div class="flex flex-col gap-4">
        ...
        <?php if (isset($article->image)) : ?>
            <div class="relative w-fit">
                ...
                <a href="<?= $hive->alias('admin_news_show', ['id' => $article->id]) ?>" class="absolute inset-0 size-full block"></a>
            </div>
        <?php else : ?>
        ...
        <div>
            <h3 class="mb-2 font-bold"><?= $article->title ?></h3>
            <div class="text-sm">
                <?= $article->summary ?>
            </div>
        </div>
    ...
    <?= component('ui/item-actions', [
        'edit_url' => $hive->alias("admin_news_edit", ['id' => $article->id]),
        'delete_url' => $hive->alias("admin_news_destroy", ['id' => $article->id]),
